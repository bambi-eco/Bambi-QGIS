...
Step 2 — Geo-referencing & Calibration
    ...
    * An analytic solver computes the z-translation and yaw correction
      (rotation Z) that make the two geo-referenced points coincide: the
      overlap point must be a circle-intersection point that both cameras
      reach with the same yaw delta.  A Gauss-Newton refinement with true
      DEM ray-casting absorbs terrain relief.  A legacy step-probe + yaw
      sweep serves as fallback.
...
class _ProbeWorker(QThread):
    ...
    def _camera_z(self, side_idx: int, correction: dict):
        s = self._sides[side_idx]
        poses = s['poses']
        fi = s['frame_idx']
        if poses is None:
        ...
        if fi >= len(images):
            return None
        loc = images[fi].get('location', [0.0, 0.0, 0.0])
        return loc[2] + correction['translation']['z']
    ...
    # ------------------------------------------------------------------
    # Analytic tz/rz solver
    # ------------------------------------------------------------------
    #
    # Perfect overlap means both geo-referenced points coincide at a single
    # world point q.  q must then be an intersection point of the two
    # circles (centre = camera XY, radius = horizontal camera-to-point
    # distance), and the yaw correction that moves point i onto q is the
    # angle from c_i→p_i to c_i→q.  Because one global yaw applies to both
    # cameras, the correct tz is the one whose circle intersection yields
    # the SAME yaw delta on both sides.
    #
    # For flat terrain the radius responds linearly to camera height:
    # r_i(Δtz) = r_i + (r_i / h_i)·Δtz  with h_i the camera height above
    # the ground point (exact for a horizontal ground plane).  This turns
    # the problem into 1-D root finding in Δtz that needs no ray-casting;
    # the roots seed a Gauss-Newton refinement that uses true DEM ray-casts
    # to absorb terrain relief.
    ...
    def _solve_tz_rz(self, corr: dict):
...
class BambiCorrectionWizard(QDialog):
    ...
    def _build_page2(self) -> QWidget:
        ...
        self._max_steps_spin.setToolTip(
            "Maximum number of ±1 m z-steps for the fallback probe\n"
            "(only used when the analytic solver fails)."
        )
        pg_layout.addWidget(self._max_steps_spin)

        self._probe_btn = QPushButton("Auto-Solve Z-Offset + Yaw")
        self._probe_btn.setToolTip(
            "Solves the z-translation and yaw correction that make the two\n"
            "geo-referenced points coincide: the overlap point must lie on the\n"
            "intersection of both circles, and both cameras must need the same\n"
            "yaw delta to reach it.  A Gauss-Newton refinement with true DEM\n"
            "ray-casting then absorbs terrain relief."
        )
    ...
    def _show_wizard_info(self) -> None:
        ...
        lbl = QLabel(
            ...
            "Use <b>Auto-Solve Z-Offset + Yaw</b> to compute the correction "
            "automatically: perfect overlap requires the common point to lie on "
